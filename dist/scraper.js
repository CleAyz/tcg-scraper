...
exports.getSnkrdunkLastSoldByGrade = getSnkrdunkLastSoldByGrade;
exports.getSnkrdunkPrice = getSnkrdunkPrice;
...
function getConfig() {
    ...
    const playwrightTimeoutMs = Number(env.PLAYWRIGHT_TIMEOUT_MS ?? 45000);
    const playwrightNavigationTimeoutMs = Number(env.PLAYWRIGHT_NAVIGATION_TIMEOUT_MS ?? 60000);
    const priceExtractTimeoutMs = Number(env.SNKRDUNK_PRICE_EXTRACT_TIMEOUT_MS ?? 20000);
    const seeMoreMaxClicks = Number(env.SNKRDUNK_SEE_MORE_MAX_CLICKS ?? 6);
    const userAgent = env.SNKRDUNK_USER_AGENT?.trim() ||
    ...
    const executablePath = executablePathFromEnv ||
        (node_fs_1.default.existsSync(defaultChromePath) ? defaultChromePath : undefined);
    return {
        baseUrl,
...
function isUsedListingsUrl(s) {
    return /\/trading-cards\/used\/listings\//i.test(s);
}
function parseCleanNumericPrice(text) {
...
function logScrapeError(message, meta) {
...
function resolveInput(raw, config) {
    const trimmed = raw.trim();
    if (!trimmed)
        throw new Error('Empty input');
    if (/^https?:\/\//i.test(trimmed)) {
        const abs = stripQueryAndHash(toAbsoluteUrl(trimmed, config.baseUrl));
        if (isUsedListingsUrl(abs))
            return { kind: 'listings', listingsUrl: abs };
    ...
    if (!config.enableSearchFallback) {
        throw new Error('Free-text search requires SNKRDUNK_ENABLE_SEARCH=true (default is true). Pass a product id, product URL, or used-listings URL.');
    }
    return { kind: 'search', query: trimmed };
}
function tradingCardIdFromHref(href) {
    const m = href.match(/\/trading-cards\/(\d+)/i);
    return m?.[1] ?? null;
}
function candidateTitleFromSearchRow(anchorTitle, anchorText) {
 ...
 */
async function resolveSearchTradingCard(page, query, config) {
    ...
    await waitForFullyLoaded(page, Math.min(config.playwrightTimeoutMs, 20000));
    const tokens = queryTokens(query);
    const queryNorm = query.trim().toLowerCase().replace(/\s+/g, ' ');
    const ranked = await page.evaluate(({ tokenList, queryNormalized }) => {
        ...
        for (const a of anchors) {
            ...
            if (!hrefVal)
                continue;
            if (!/\/trading-cards\/\d+/i.test(hrefVal))
                continue;
            if (/\/used\//i.test(hrefVal))
                continue;
            const text = (a.textContent || '').trim();
            const anchorTitle = (a.getAttribute('title') || '').trim();
            ...
            if (tokens.size)
                s += Math.round((matched / tokens.size) * 6);
            if (queryNormalized && blob.includes(queryNormalized))
                s += 12;
            if (/\/trading-cards\/\d+/i.test(hrefVal))
                s += 3;
            if (s > 0)
                rows.push({ href: hrefVal, score: s, text: text.slice(0, 400), anchorTitle });
        }
...
async function extractTitleFromPage(page) {
    const title = await page.evaluate(() => {
        ...
        if (main)
            return (main.textContent || '').trim().split('\n')[0]?.trim() || '';
        return '';
    });
    return title;
}
async function findUsedListingsUrlFromProductPage(page, config) {
    return await page.evaluate(() => {
        const anchors = Array.from(document.querySelectorAll('a[href*="/trading-cards/used/listings/"]'));
        if (!anchors.length)
            return null;
        let best = null;
        let bestScore = -999;
        for (const a of anchors) {
            ...
            for (let depth = 0; depth < 12 && el; depth++) {
                ...
                if (cls.includes('similar'))
                    score += 6;
                if (txt.includes('similar items'))
                    score += 6;
                if (cls.includes('used-listing'))
                    score += 4;
                if (cls.includes('product-item'))
                    score += 1;
                el = el.parentElement;
            }
            if (score > bestScore) {
        ...
        return href || null;
    }).then((href) => (href ? stripQueryAndHash(toAbsoluteUrl(href, config.baseUrl)) : null));
}
async function clickSeeMoreWhilePresent(page, maxClicks) {
 ...
 */
async function extractLastSoldByGradeFromListingsPage(page) {
    const raw = await page.evaluate(() => {
        const items = Array.from(document.querySelectorAll('[class*="product-item"]'));
        const rows = [];
        let order = 0;
    ...
    for (const row of raw) {
        if (byGrade[row.grade] !== undefined)
            continue;
...
async function inferProductUrlFromListingsPage(page, config) {
    const href = await page.evaluate(() => {
        const anchors = Array.from(document.querySelectorAll('a[href]'));
        for (const a of anchors) {
            const h = a.getAttribute('href') || '';
            if (!/\/trading-cards\/\d+/i.test(h))
                continue;
            if (/\/trading-cards\/used\/listings\//i.test(h))
                continue;
...
async function getSnkrdunkLastSoldByGrade(cardQuery) {
    const config = getConfig();
    if (!cardQuery?.trim())
        throw new Error('getSnkrdunkLastSoldByGrade requires a non-empty query');
    ...
    try {
        ...
        if (resolved.kind === 'listings') {
            ...
            if (!foundListings) {
                throw new Error(`Could not find a used listings link on product page. Open the card on SNKRDUNK and paste the full /used/listings/... URL.`);
            }
 ...
 */
async function getSnkrdunkPrice(cardName) {
    const r = await getSnkrdunkLastSoldByGrade(cardName);
    if (r.type === 'AMBIGUOUS') {
        throw new Error('Search is ambiguous: multiple products matched. Pick a candidate productUrl and call again with that URL or id.');
    }
    if (r.lastSoldByGrade['PSA 10'] != null)
        return r.lastSoldByGrade['PSA 10'];
    const first = r.lastSoldGradeOrder[0];
    if (first != null && r.lastSoldByGrade[first] != null)
        return r.lastSoldByGrade[first];
    const keys = Object.keys(r.lastSoldByGrade).sort();
    if (!keys.length)
        throw new Error('No sold prices found');
